...
5. **The Publisher may read Settings and a folder of finished files, may
   ...
   which is published — so it never reaches the Store and undo stays the
   Face's sequence (rule 1).
...
Every published edit is recoverable in one step (S9), which is what
...
kept beside them. **It does not reach a photograph's original**, which never goes to
