...
fixed page, template or furniture file it does not hold is kept beside
them. **It does not reach a photograph's original**, which never goes to
...
sits at the repository root and writes the result into Settings — and the
Face offers that same action afterwards**, because a file added to the
