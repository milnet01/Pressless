...
back into the Store, rebuild, publish. **An entry the fetched state does
not hold becomes a draft rather than being deleted** — it was published
once and it is still his. It ends with the site and his own files
...
Publisher consults.** At publish it removes a root entry absent from the
folder it was handed unless that entry is on the list. It never
...
asked to remove. **So a file added to the repository root outside
Pressless is unprotected until the list is derived again**, and setup is
not the only moment that may happen.
...
| **Pressless's own folder**, outside the site folder | All his writing — drafts and published entries alike, kept apart — photograph originals, the settings file, the rolling log, the Insights cache — and, only where there is no keyring, the credential file ADR-0003 falls back to, owner-readable and nothing else | **Never** |
