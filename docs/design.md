...
|---|---|---|
| **Settings** | What is true of this machine and this site rather than of his writing: where the site folder is, which repository to publish to, the Daily Prompt filter, the untouchable list, where **both** his secrets are kept, and the Analytics property id — the number Google's reporting interface is queried by, never the `G-…` measurement id the site's footer carries. | Everything. It depends on nothing. |
...
**And the Publisher applies the rule again at every publish.** So a file
appearing at the root after setup is left alone because the Builder did
not produce it, not because it is on the list. A list derived once and
trusted forever is how the next verification token gets deleted.
