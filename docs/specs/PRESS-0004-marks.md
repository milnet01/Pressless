...
rule instead (§4.2); applied there they would reject `{photo:
seaside.jpg}` on the space after `{photo:`, and the photo row's own
`example` would fail INV-6. The clauses
