...
| That these three writers emit the wider-grant notice | **nothing here** — PRESS-0005's falsifier exercises `write`, an entry call, and reaches none of them. An implementer writing `write_html`, `write_template` or `write_comments` with a plain `open` loses the notice while every invariant in §5 still passes. Filed as PRESS-0115 |
