...
that cannot be parsed raises `StoreError` naming the file by its own name.

**No message raised, and no notice emitted, here names a full path.** `docs/design.md` § Logging
puts that on the part that RAISES, so it is this module's rule, and
PRESS-0005 § 4.4 states it for the shared code. A file is named by its own
name, never by the folder it sits in, and an `OSError` is reported by its
reason rather than by its own words, which quote the path it failed on.

**Deleting a page, template, furniture or comments file is PRESS-0005's
`move_to_bin`**, which takes any path inside the Store's own sub-folders.
Nothing here unlinks a file.
...
(§4.1). So an implementation filtering on the name rule alone passes the
...
- **INV-6** — Comments survive `write_comments` then `read_comments`
  with every field and their order unchanged, `date` to the resolution
  §4.2's format holds. **A fraction of a second is dropped and is NOT
  refused** — measured, `…:05.123456` reads back `…:05` — because it
  misorders nothing and refusing it would reject `datetime.now()`. INV-12
  refuses an offset for the opposite reason: an offset changes which
  instant the value names.
...
- **INV-10** — A comments file is UTF-8 with LF line endings whatever
  ...
  *Breaks when:* an implementer names no `newline` at all. On Windows the
  platform then translates the module's own `\n` to CRLF and a body's CRLF
  to CR CR LF. **Naming `""` or `"\n"` is not the difference** — on write
  Python treats them identically (measured, byte-for-byte), so the page's
  `""` records intent rather than behaviour, and unifying the two values
...
| The folder handed in is not a folder | `StoreError` saying so, never naming its path. That folder is the caller's rather than the Store's, so a mistyped one is an error and never an empty listing |
| No page or furniture file at that path | `StoreError` naming the file by its own name. Unlike an absent comments file, absence here is not the ordinary case: a page the Builder asks for and cannot find is a fault rather than an empty set |
...
| A comments file is not valid JSON | `StoreError` naming the comments file by its own name. It is never rewritten into something parseable |
| A comments file holds a field the record does not have | `StoreError` naming the comments file by its own name, and the field. Unlike an entry's unknown header field, which ADR-0001 keeps, an unexpected field here is most likely one this spec forbids |
| A comments file is missing one of the record's fields | `StoreError` naming the comments file by its own name, and the field. The pair with the row above: a record is the whole set and nothing else, so neither an extra nor an absence is read past |
