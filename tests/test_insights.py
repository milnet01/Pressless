...
def test_every_request_carries_a_timeout():
    ...
    with pytest.raises(OSError):
        client.request("GET", f"{insights_module.API}/x", None, {})

    assert opener.timeouts == [insights_module.TIMEOUT_SECONDS], (
        f"the request was made with timeout {opener.timeouts!r}, not the "
        f"module's {insights_module.TIMEOUT_SECONDS!r}"
    )
    assert insights_module.TIMEOUT_SECONDS > 0, (
...
def test_a_broken_reply_reaches_the_caller_as_oserror(broken):
    ...
    assert 'THE-ACCESS-TOKEN' not in str(raised.value), (
